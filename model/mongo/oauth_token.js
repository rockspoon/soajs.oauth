...
function Oauth(service, options, mongoCore) {
  ...
  if (!__self.mongoCore) {
    ...
    //   console.log('registry.coreDB.oauth');
    //   console.log(JSON.stringify(registry.coreDB.oauth));
...
Oauth.prototype.delete = function (data, cb) {
  ...
  if (
    ...
    !(
      ...
      data.clientId ||
      data.user ||
      (data.user && data.user.id && data.user.loginMode)
    )
  ) {
    let error = new Error(
      '(token and type) or clientId or user[id, loginMode] is required.'
    );
